...
def validate(s):
    ...
    if m.get('stable') is not True or m.get('status')!='stable-prototype': e.append('v2.2 stable prototype metadata')
    ...
    required_gate=('human_rules','machine_spec','validator','core_validation_pass','rulebook_pdf','cards_pdf','impassable_marker_pdf','visual_pdf_review','source_pdf_crosscheck','final_release_lock')
    if not all(gate.get(k) for k in required_gate): e.append('stable release gate')
...
def main():
    ...
    print('SONUC: PASS')
    print('- v2.2 stabil prototip / v2.1 baseline sözleşmesi doğru.')
