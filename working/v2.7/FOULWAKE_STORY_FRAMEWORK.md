# FOULWAKE - hikâye omurgası ve ses kılavuzu

**8 Eylül 2026 · Uygulanmış v2.7 çalışma metni · kilitli sürüm değildir.**

Bu kayıt artık boş sahne şablonu değildir. Tam anlatı `FOULWAKE_RULEBOOK_STORY_v2.7.md`, oynama sırası `FOULWAKE_KURAL_KITABI_v2.7.md`, kartların asıl metni kart JSON'larıdır. Başeditör, proje sahibinin güncel talimatıyla Hikâye Editörü, Simülasyon QA, Sanat Yönetimi ve Görsel Uygulama görevlerini bu teslim için üstlenmiştir. Önceki CAN-07'nin yalnız küçük öneri üretme sınırı bu açık revizyon talimatıyla aşılmıştır; kilitli v2.6 yeniden yazılmamıştır.

## Korunan dünya ve değişen ad

| Kimlik | Bu sürümdeki sınır |
|---|---|
| CAN-01 | 1721'in maddi zemini: yelken, sağlık kâğıdı, borç, tiryak, karantina ve vardiya. Modern teknoloji veya tıp bilgisi karakterlere aktarılmaz. |
| CAN-02 | Arden Krallığı eve dönüş yeridir; San Cordelio ecza rıhtımı kalkış, Saint Verena Karantina Limanı teslim eşiğidir. Eleonora, Mattias Veyr ve Gusto Varela korunur. |
| CAN-03 | Eski Siyah Mühür adı yerine **Kuru Pay** kullanılır. Bu, önce kıyıda kalanların kazancına verilen lakaptır; tek merkezli bir örgütün doğrulanmış adı değildir. |
| CAN-04 | Gusto'nun akıbeti, eksik sayfaların faili ve balmumunun ilişkisi bilinmez. Moderatör gizlice bir doğru cevap seçmez. |
| CAN-05 | Kara komedi, çıkarın ve gündelik alışkanlığın tehlikede bile sürmesinden doğar. Acı çeken kişiyi şakanın hedefi yapmaya gerek yoktur. |
| CAN-06 | Her kartın sonuna aynı bilgece ters köşe eklenmez. Bazısı somut davranış, bazısı kısa konuşma, bazısı da sessiz bir ayrıntı taşır. |
| CAN-07 | 7 Eylül yetkisiyle açılış, arka plan, kart metinleri ve tam kural kitabı birlikte düzenlendi. Sonraki değişiklikte bağlı kaynaklar ve üretim dosyaları birlikte güncellenir. |
| CAN-08 | Araştırma tarihsel zemin sağlar. Dünya ve olay örgüsü FOULWAKE'a aittir. Başka eserlerden ad, uzun alıntı veya olay zinciri taşınmaz. |
| CAN-09 | Tat metni, özel geçmiş ve atmosfer ek mekanik değildir. Gizli bilgi yalnız yazılı bakış/açma pencerelerinden gelir. |

Bunlar güncel çalışma sürümünün editoryal sınırlarıdır; belgeyi yayımlanmış yeni kanon veya fiziksel üretim kabulü yapmaz.

## Masadaki gerçek anlar

| Kimlik | Kaynak | İşlev ve geçiş |
|---|---|---|
| SCN-01 | §3.1 | Oyuncu yük defterinde adı olan sorumlu heyettir. Dünya açıklamasından önce kendi işini ve varış yerini öğrenir. Karakter dağıtımına geçilir. |
| SCN-02 | §3.3 | Gelgit, karantina ve yeniden ücret baskısı Gusto'yu bekleyememeyi açıklar. Kefalet, Kaptan seçimini zorunlu kılar. Gizem ayrı bir araştırma turuna dönüşmez. |
| SCN-03 | §3.5 | Kaptan eksik defterden tek rota bilgisi alır. “Gece” burada kurulumun kapalı göz aşamasıdır; hikâyede fazladan bir gün beklenmez. |
| SCN-04 | §3.6 | Oyuncu özel gerekçesine sessizce bakar. Sadakat ile Karakter bağımsızdır. Zorunlu tanışma veya itiraf turu yoktur. |
| SCN-05 | §3.7-3.8 | İlk rota birlikte seçilir. İlk yolculuk gecesinde uygun Karakterler çalışır; Hainler en son tanışır. İlk tarafsız geceyle karıştırılmaz. |
| SCN-06 | §4 | Gündüz tartışması, olay ve gece döngüsü. Anlatıcı yeni kriz eklemez; krizi kartlar ve oyuncuların tercihleri üretir. |
| END-01 | §4.4 | Limana giriş teslim değildir. Son gece, kıyı görünürken aynı gizli sadakat gerilimini sürdürür. |
| END-02 | §4.4 | Yalnız gerçekleşen takım sonucu okunur. İlaç kesin çare olmaz; Hainlere verilen sözlerin tutulduğu ilan edilmez. |

## Kişiler ve çıkarları

- **Eleonora:** Halkı kurtarmak ve ticaret ailelerine bağımlılığını azaltmak ister. İkisi birbirini dışlamaz.
- **Veyr:** Denemelerin sınanmasını ister. Başarısız kayıtları da tutar; kesin başarı sözü vermez.
- **Gusto:** Vekilsiz yönetim alışkanlığı, yokluğunda sefer heyetini sorumlu bırakır. Bu kusur onun suçunu kanıtlamaz.
- **Kuru Pay'dan kazananlar:** Depo, karantina hizmeti, borç ve aracılık üzerinden çıkar sağlar. Ortak çıkar, tek efendi veya ortak emir kanıtı değildir.
- **Sefer heyeti:** Eve dönmek, geçinmek, birine sözünü tutmak, bir kaybı önlemek. Aynı kişisel istek karşıt sadakatlerde görülebilir.

## Kart sesleri

Uzakgören mesafe üzerinden kendini savunur; Kıyıçizen çizginin düzenine sığınır; Karga Yuvası başkasını ikna etmeye çalışır. Canhalatı düğümlere, Tahtakakan el işine güvenir. Özel yeteneği olmayan sekiz karakterin tartışmaya katılma nedeni vardır; bunlar yeni yetenek kazanmaz.

On Tayfa gerekçesi de beş Hain gerekçesi kadar kişiseldir. Hastalık veya borç sözü tek başına Hain işareti olmaz. Hainler sırayla eczacı hesabı, karantina hanında rehin tutulan aile, sahte sağlık kâğıdı, eksik bir belgeye duyulan inanç ve kendi kayıp kaydı üzerinden baskı altındadır. Bunlar sabit takım hedefini açıklayan özel metinlerdir; tamamlanacak yan görev değildir.

Dört küçük gönderme yalnız nesne veya durum düzeyindedir: emanet şapka, sahibinin isteğiyle sorun yaşayan pusula, adı yazılmamış prens buyruğu, tabut için kesilip gemide kullanılan tahta. Bunları oyuncuya açıklamak veya başka kartlarda tekrar etmek gerekmez. Kaynak ve tercih gerekçesi QA araştırma notundadır.

## Uygulama disiplini

Atmosfer cümlesi kısa tutulur, ardından yazılı olay uygulanır. Oyuncunun konuşması ve kararı anlatıcı tarafından doldurulmaz. §17 merak edenler içindir. Görsel ve yerleşim kontrolü 8 Eylül owner yetkisiyle bu teslimin kapsamındadır. Güncel sanat kararı FOULWAKE_CURRENT_ART_DIRECTION_v2.7.md içindedir; kabul edilmiş KAPTAN figürü kendiliğinden Gusto yapılmaz.

Bu revizyonun değişiklikleri ve kaynak karşılaştırmaları `FOULWAKE_EDITORIAL_DECISIONS_v2.7.json` ve `qa/editorial_revision/` altında izlenir. Tarihsel 20 Ağustos hikâye PASS'i önceki committe korunur; yeni metnin insanlarla denendiği anlamına gelmez.

8 Eylül tam kontrolü: `qa/whole_game_polish_20260908/review.md`. Sözcük, kart ve baskı düzeltmeleri burada kayıtlıdır; adların anlatısal işlevi korunur.
