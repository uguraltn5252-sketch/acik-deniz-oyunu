...
OUT=P/'publication_20260909'
ASSETS=P/'visual/illustrated_design_20260908/assets'
fontroot=OUT/'fonts'
for name,suffix in [('FWBody','Regular'),('FWBold','Black'),('FWItalic','Italic'),('FWBoldItalic','Bold')]:
    pdfmetrics.registerFont(TTFont(name,str(fontroot/f'Alegreya-{suffix}.ttf')))
pdfmetrics.registerFontFamily('FWBody',normal='FWBody',bold='FWBold',italic='FWItalic',boldItalic='FWBoldItalic')
INK=colors.CMYKColor(0,0,0,1)
...
def metadata(r):
    ...
    if k=='provisions':return r['time']+' · Gerçek Güç değildir'
...
def print_image(path):
    # Prepress encoding at >=300 ppi; illustration master, color and crop unchanged.
    # The lossless generated PNG is retained beside every final card.
    stream=BytesIO()
    with Image.open(path) as im:
        im=im.convert('RGB')
        r=INDEX[Path(path).stem];w,h=dimensions(r)
        target=(math.ceil(w/72*300),math.ceil(h/72*300))
        if im.width>target[0] and im.height>target[1]:im=im.resize(target,Image.Resampling.LANCZOS)
        im.save(stream,'JPEG',quality=93,subsampling=0,optimize=True)
    stream.seek(0)
...
def draw_front(c,r,x,y,scale=1,annotation=None,bleed=True):
    ...
    # Actual glyph outlines, including the dotted capital İ, keep 4.5 mm trim clearance.
    if r['id'] in ('GUC-17','HAR-KY-01'):ty-=.3*mm
    ...
    def copy_paragraphs(bs,fs):
        ep=para(r['effect'],bs,align=0 if len(r['effect'])>120 else 1);_,eh=ep.wrap(cw,10000)
    ...
    panel_top=h*(1-panel)
    # New native reading panel replaces only the copy area, never any illustration.
    # This is page composition, not raster retouching. Preserve the engraved perimeter.
    c.setFillColor(colors.HexColor('#F3E4C8'))
    c.roundRect(3.8*mm,6.3*mm,w-7.6*mm,panel_top-7.1*mm,1.1*mm,fill=1,stroke=0)
