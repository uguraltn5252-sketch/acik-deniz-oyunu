...
from book_layout import build_book
R=Path(__file__).resolve().parents[4]
P=R/'working/v2.7'
OUT=P/'visual/polish_20260908';PDF=OUT/'pdf';PDF.mkdir(parents=True,exist_ok=True)
...
class Sources:
 def __truediv__(self,name):
  new=OUT/'assets'/name
  prior=P/'visual/full_deck_20260907/assets'/name
  return new if new.exists() else prior if prior.exists() else P/'visual/recheck_20260907/assets'/name
layout.ASSETS=Sources();layout.PDF=PDF;layout.OUT=OUT;layout.QA=P/'qa/whole_game_polish_20260908'
QA=layout.QA;ROOT=R
ARTIFACTS=R.parent/'artifacts/polish_20260908'
records=layout.records;INDEX=layout.INDEX;BACKS=layout.BACKS;CAPTAIN=layout.CAPTAIN
dimensions=layout.dimensions;back_id=layout.back_id
_original_rich=layout.rich
layout.rich=lambda text: _original_rich(text).replace('\n','<br/>')
_original_metadata=layout.metadata
def metadata(r):
 if r['collection']=='characters' and r.get('role','').strip() in ('','-','—'):
  return f"Etki {r['impact']} · {r['use_mode']}"
 return _original_metadata(r)
...
def back(c,bid,x,y,w,h,bleed=True):
 ...
 if bid=='BACK_LIGHTHOUSE':
  # Only the lower sea uses the local image edit. The liked upper scene is
  # placed directly from the unchanged 20260907 source, at the same scale.
  split=650/1254
  for path,lo,hi in [(OUT/'assets/LIGHTHOUSE_SEA_FILL.png',0,1-split),
                     (P/'visual/full_deck_20260907/assets/BACK_LIGHTHOUSE.png',1-split,1)]:
   c.saveState();clip=c.beginPath();clip.rect(0,h*lo,w,h*(hi-lo));c.clipPath(clip,stroke=0)
   layout.fitted_image(c,path,0,0,w,h,contain=False);c.restoreState()
 else:
...
def instructions(c):
 ...
 paragraphs=[
  'Bu destenin 121 kartının tamamı resimlidir. Güncel kart metinleri, yedi ortak arka yüz ve tek kuleli fener bu dosyada birlikte yer alır.',
...
def build_review():
 ...
 boxtext(c,'Tayfa/Hain ve Deniz/Kayalık kendi aileleri içinde aynı masterı kullanır. Fenerin tek kuleli yandan görünümü dört fener kartında ortaktır. Bu sayfalardaki etiketler kart arkasında yer almaz.',18*mm,46*mm,174*mm,10)
...
def build_all():
 ...
 cards=build_cards();review=build_review();lighthouse();book=build_book(layout)
 import shutil
 shutil.copy2(book,ARTIFACTS/book.name)
 volumes=split_pdf(cards,'FOULWAKE_KARTLAR',pairs=True)+split_pdf(review,'FOULWAKE_GORSEL')
 manifest={'task_id':'FOULWAKE-WHOLE-GAME-POLISH-001','status':'GENERATED_AWAITING_RENDER_INSPECTION',
 'cards':layout.PLACEMENTS,'card_pdf_positions':layout.PAGE_RECORDS,'copy_geometry':geometry,
 'back_counts':dict(Counter(back_id(r)for r in records)),
 'lighthouse':{'upright_towers':1,'top_original_source_rows':[0,650],'canvas_px':[1254,1254],'rotation_degrees':[0,0],'lower_layer':'assets/LIGHTHOUSE_SEA_FILL.png'},'volumes':volumes,
 'compiled_masters':{p.name:{'pages':len(PdfReader(p).pages),'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}for p in (cards,review,ARTIFACTS/book.name)},
